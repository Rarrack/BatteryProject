...
public class GoalSettings : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            if (collision.gameObject == bat && (bat.transform.localEulerAngles.z >= transform.localEulerAngles.z - 5 && bat.transform.localEulerAngles.z <= transform.localEulerAngles.z + 5) && (bat.GetComponent<ObjectTouchMovement>().charged == true || bat.GetComponent<ObjectMovement>().charged == true))
            {
                c.isTrigger = true;
